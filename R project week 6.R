...
screen_types_in_theater <- function(theater_num , screens_type, screens , seats , movies , time, movie_rating){
  
  week_days <- rep(0, 7)  # Store totals for each day
  total_revnues_per_day <- rep(0, 7)
  total_screen_type_revenue <- rep(0, 7)
  screens_type <-
    data.frame(
      type = c('3D' , 'VIP' , 'Standard'),
      price = c(75 , 150 , 55),
      seats = c(100, 30, 200)
    )
  ...
  for (day in 1:length(week_days)) {
    # Keep track of total revenue for the day
    total_revenue_of_screen_types <- 0
    
    # iterate through the amount of screens on a particular day
    for (s in 1:screens) {
      # Calculate  how many visitors are watching the movie
      visitors_of_screens <- 0
      
      
      # check the rating for each movie
      if(movie_rating[s] == 'PG-13' | movie_rating[s] == 'R' | movie_rating[s] == 'Not Rated'){

        visitors_of_screens = sample(seats, 1)
        revenue = calculate_revenue(visitors_of_screens = 0,  price , type)
      }else{
        # both visitors_of_screens watch 
        visitors_of_screens = sample(seats, 1)
        seats_remaining <- seats - visitors_of_screens
        revenue = calculate_revenue(visitors_of_screens , price , type)
      }
...
#do time function inside the function 

screen_types_in_theater(theater_num = 1 , screens_type = 'VIP' , seats = 30  , movies, time = '2pm' , movie_rating)
